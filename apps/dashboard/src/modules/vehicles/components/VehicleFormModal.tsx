...
import { createVehicle, updateVehicle } from '@/api/mutations/vehicles.mutation';
...
const STATUS_OPTIONS = ['active', 'idle', 'stopped', 'offline', 'maintenance'] as const;
...
const schema = z.object({
  ...
  fuelConsumptionNorm: z.coerce.number().min(0),
      ...
      <div className="relative w-full max-w-2xl bg-[#0F1C30] border border-[#1E3150] rounded-xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit((v) => mutation.mutate(v))} className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Device IMEI" error={errors.deviceImei?.message} required>
              <input
                {...register('deviceImei')}
          ...
          <div className="grid grid-cols-3 gap-4">
            <Field label="Make" error={errors.make?.message} required>
              <input {...register('make')} placeholder="Toyota" className={inputClass} />
            </Field>
            <Field label="Model" error={errors.model?.message} required>
              <input {...register('model')} placeholder="Camry" className={inputClass} />
            </Field>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <Field label="Tank Capacity (L)" error={errors.fuelTankCapacity?.message} required>
              <input
            ...
            <Field label="Consumption Norm (L/100km)" error={errors.fuelConsumptionNorm?.message} required>
              <input
                {...register('fuelConsumptionNorm')}
                type="number"
                step="0.1"
                placeholder="10"
                className={inputClass}
          ...
          <Field label="Assigned Driver" error={errors.currentDriverId?.message}>
