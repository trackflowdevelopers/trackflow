...
import { dateRange, todayLocalISO } from "../utils/functions";
...
export function RouteHistoryPage() {
  ...
  const liveUpdate =
    isToday && vehicleId ? liveUpdates.get(vehicleId) : undefined;
  ...
  useEffect(() => {
    ...
    setLivePoints((prev) => {
      ...
      const next: RoutePoint = {
        ...
        ignition:
          liveUpdate.status === "active" || liveUpdate.status === "idle",
        ...
        <div>
          ...
          {selectedVehicle && (
            <p className="text-[#8ba3c0] text-sm mt-0.5">
              {selectedVehicle.plateNumber} · {selectedVehicle.make}{" "}
        ...
        <div className="flex items-center gap-3">
          {isToday && vehicleId && (
            ...
            >
              <Radio
                size={14}
                className={followLive && liveUpdate ? "animate-pulse" : ""}
              />
        ...
        <div className="bg-[#0F1C30] border border-[#1E3150] rounded-xl p-4 overflow-hidden">
          {route ? (
            ...
            <div className="h-full flex items-center justify-center text-[#8ba3c0] text-sm text-center">
              {vehicleId
                ? "Loading…"
                : "Pick a vehicle and date to see the timeline."}
            </div>
          )}
        </div>
      </div>
